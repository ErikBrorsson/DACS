...
def get_arguments():
    ...
    parser.add_argument("--save_output_images", action="store_true", default=True,
...
def colorize_image(image, epoch, id, palette):
    with torch.no_grad():
        if image.shape[0] == 3:
            ...
            # image.save(os.path.join(pred_dir, id + '.png'))
        else:
            mask = image
            colorized_mask = colorize_mask(mask, palette)
            return colorized_mask
            # colorized_mask.save(os.path.join(pred_dir, id + '.png'))
...
def evaluate(model, dataset, ignore_label=250, save_output_images=False, save_dir=None, input_size=(512,1024)):
    ...
    for index, batch in enumerate(testloader):
        image, label, x, y, image_name = batch
        # size = size[0]
        if index > 500:
           break
        with torch.no_grad():
            ...
            if args.save_output_images:
                if not os.path.exists(pred_dir):
                    os.mkdir(pred_dir)
                image_color = colorize_image(image[0],index, "we", palette.CityScpates_palette)
                gt_color = colorize_image(gt,index,image_name[0],palette.CityScpates_palette)
                pred_color = colorize_image(output,index,image_name[0],palette.CityScpates_palette)
                image_and_gt = get_concat_h(image_color, gt_color)
                image_gt_pred = get_concat_v(image_and_gt, pred_color)
                image_gt_pred.save(os.path.join(pred_dir, image_name[0] + '.png'))
